...
segmentbis <- function (Y_qc, Yhat, optK, K, sampname_qc, ref_qc, chr, lmax, mode){
    ...
    for (sampno in 1:ncol(Y_qc)) {
        ...
        lratio <- (1 - chat/2) * lambda + log((chat + 1e-04)/2.0001) * 
            yact
        #chat[chat > 5] <- 5 #to obtain the real duplication number
        if (sum(lratio > 0) > 0) {
            if (sum(lratio > 0) >= 2) {
                finalmat <- (cbind(i, j, yact, lambda, chat, 
                  lratio))[lratio > 0, ]
                ...
                while (s <= (nrow(finalmat))) {
                  ...
                  rowend <- finalmat[s, 2]
                  rowsel <- (finalmat[, 1] <= rowend & finalmat[, 
                    2] >= rowstart)
                  rowsel[s] <- FALSE
            ...
            for (s in 1:nrow(finalmat)) {
                tau <- sort(unique(c(as.vector(finalmat[1:s, 
                  1:2]), 1, num)))
                P <- length(tau) - 2
                mbic <- loglikeij[s]
    ...
    if (nrow(finalcall) == 1) {
        finalcall <- t(as.matrix(c(finalcall[, 1:2], cnvtype, 
            st, ed, (ed - st + 1)/1000, finalcall[, 3:9])))
    }
    else {
        finalcall <- cbind(finalcall[, 1:2], cnvtype, st, ed, 
            (ed - st + 1)/1000, finalcall[, 3:9])
    }
    ...
    pvalue = signif(pchisq(q = lratio * 2, df = 1, lower.tail = FALSE), 
        3)
...
# write results
write.table(finalcall, file = paste(projectname,'_',cur_chr,'_', optK,'_',mode,'.txt', sep=''), sep='\t', quote=FALSE, row.names=FALSE)
